...
function buildXrosLoader() {
  const ic = {
    xros: `${XL}/XrosLoader.png`,
    cherry: `${XL}/Cherry_Blossom-Xros_Loader.png`,
    decid: `${XL}/Decidious-Xros_Loader.png`,
    digi: `${XL}/Digicode.png`,
    piece: `${XL}/Digicode_Piece.png`,
    coin: `${XL}/Coin_Currency.png`,
  };

  const craftLevels: [string, string, number, string][] = [
  ...
  return {
    ...
    layout: [
      ...
      {
        blockType: 'imageGrid', title: 'Aura Preview', columns: '2',
        images: [
      ...
      {
        blockType: 'imageGrid', title: 'Item Preview', columns: '2',
        images: [
          { caption: 'Cherry Blossom-Xros Loader', imageUrl: ic.cherry },
          { caption: 'Decidious-Xros Loader', imageUrl: ic.decid },
        ],
      },
      {
        blockType: 'table',
        ...
        rows: [
          { cells: [
            { value: 'Digimon Xros Loader Lv 0', iconUrl: ic.xros },
            { lines: [{ text: 'Digicode x1', iconUrl: ic.digi }] },
            { value: '-' },
            { value: '100%' },
          ] },
          ...craftLevels.map(([to, from, pieces, cost]) => ({
            cells: [
              { value: `Digimon Xros Loader ${to}`, iconUrl: ic.xros },
              { lines: [
                { text: `Digimon Xros Loader ${from}`, iconUrl: ic.xros },
                { text: `Digicode Piece x${pieces}`, iconUrl: ic.piece },
              ] },
              { value: cost },
              { value: '100%' },
            ],
          ...
          { cells: [
            { value: 'Cherry Blossom / Decidious-Xros Loader', iconUrl: ic.cherry },
            { lines: [
              { text: 'Digimon Xros Loader Lv 10', iconUrl: ic.xros },
              { text: 'Digicode Piece x98', iconUrl: ic.piece },
            ] },
            { value: '227,386,336' },
            { value: '100%' },
          ] },
      ...
      },
      {
        ...
        rows: [
          { cells: [{ value: 'Digicode Piece', iconUrl: ic.piece }, { value: '363' }] },
          { cells: [{ value: 'Money', iconUrl: ic.coin }, { value: '898,928,970' }] },
        ],
      },
    ],
  };
...
function buildAdventureGoggles() {
  const ic = {
    box: `${AG}/Adventure_goggles_box.png`,
    g1: `${AG}/Adventure_goggles1.png`,
    core: `${AG}/Contaminated_X-Antibody_-_CORE.png`,
  };
  ...
  function goggleRows(statName: string, data: (string | number | null)[][]) {
    return data.map(([level, skillDmg, stat1, stat2, totalSkill, totalStat]) => ({
      cells: [
        { value: String(level), iconUrl: String(level) === 'Adventure Goggles Box' ? ic.box : ic.g1 },
  ...
  const craftRows: [string, number, string][] = [
  ...
  return {
    ...
    layout: [
      ...
      { blockType: 'callout', type: 'info', content: [p('The crafting NPC is Dorumon (Goggles Make) who is located near V-mon and Digitamamon.')] },
      { blockType: 'callout', type: 'warning', content: [p('There is a chance to fail when leveling up. You may lose your money, but the goggles stay at the same level. You can reroll until you get the stats you are looking for.')] },
      {
        blockType: 'imageGrid', title: 'Goggle Variants', columns: '4',
        images: [
      ...
      {
        blockType: 'imageGrid', title: 'Equipment Slot & Rerolling', columns: '2',
        images: [
      ...
      },
      {
        ...
        rows: craftRows.map(([name, cores, cost]) => ({
          cells: [
            { value: String(name), iconUrl: ic.box },
            { lines: [{ text: `Contaminated X-Antibody - CORE x${cores}`, iconUrl: ic.core }] },
            { value: cost },
          ],
        })),
      },
      ...
      { blockType: 'table', title: 'HT/HT Goggles (Hit Rate)', headers: [{ label: 'Item' }, { label: 'Status Given' }, { label: 'Total Status' }], rows: goggleRows('Hit Rate', htData) },
      { blockType: 'table', title: 'AT/AT Goggles (Attack)', headers: [{ label: 'Item' }, { label: 'Status Given' }, { label: 'Total Status' }], rows: goggleRows('Attack', atData) },
      { blockType: 'table', title: 'CT/CT Goggles (Critical Hit)', headers: [{ label: 'Item' }, { label: 'Status Given' }, { label: 'Total Status' }], rows: goggleRows('Critical Hit', ctData) },
    ],
  };
}
